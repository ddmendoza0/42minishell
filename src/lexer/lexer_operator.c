...
/*   Updated: 2025/10/03 16:53:40 by dmaya-vi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_triple_operator(const char *input, size_t i, size_t op_len,
        t_shell *shell)
{
    char	next_char;
    char	prev_char;
...
int	extract_oprtr(size_t *i, const char *input, t_token **lst, t_shell *shell)
{
    ...
    if (!op)
        return (0);
    op_len = *i - start_i;
    if (!check_triple_operator(input, *i, op_len, shell))
    {
        free(op);
        return (0);
    }
