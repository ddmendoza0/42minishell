...
static int	expand_command_redirections(t_command *cmd, t_shell *shell)
{
		...
		&& cmd->input_redir->original_token)
	{
		...
		// Valida archivo de entrada si no es heredoc
		if (!cmd->input_redir->is_heredoc
			&& cmd->input_redir->expanded_path
			&& *cmd->input_redir->expanded_path)
		{
			test_fd = open(cmd->input_redir->expanded_path, O_RDONLY);
			if (test_fd == -1)
			{
				print_error("minishell", cmd->input_redir->expanded_path,
					strerror(errno));
